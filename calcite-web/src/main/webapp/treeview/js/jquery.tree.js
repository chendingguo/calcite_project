...
(function($) {
	...
	$.fn.treeview = function(settings) {
		...
		function buildnode(nd, ht, deep, path, isend) {
			...
			ht.push("<div id='", id, "_", nid, "' tpath='", path,
					"' unselectable='on' title='", nd.text, "'");
			var cs = [];
			...
			if (deep == 1) {
				ht.push("<img class='bbit-tree-icon' src='",
						dfop.emptyiconpath, "'/>");
			} else if (deep > 1) {
				ht.push("<img class='bbit-tree-icon' src='",
						dfop.emptyiconpath, "'/>");
				for (var j = 1; j < deep; j++) {
					ht.push("<img class='bbit-tree-elbow-line' src='",
							dfop.emptyiconpath, "'/>");
				}
			}
			ht.push("</span>");
			...
			if (nd.hasChildren) {
				if (nd.isexpand) {
					...
					cs.push(isend ? "bbit-tree-elbow-end-plus"
							: "bbit-tree-elbow-plus");
				}
			} else {
				cs.push(isend ? "bbit-tree-elbow-end" : "bbit-tree-elbow");
			}
			ht.push("<img class='bbit-tree-ec-icon ", cs.join(" "), "' src='",
					dfop.emptyiconpath, "'/>");
			ht.push("<img class='bbit-tree-node-icon' src='",
					dfop.emptyiconpath, "'/>");
			//checkbox
			...
			if (nd.href) {
				ht
						.push("<a hideFocus class='bbit-tree-node-anchor' tabIndex=1 href='"
								+ nd.href + "'>");
			} else {
				ht
			...
			if (nd.hasChildren) {
				if (nd.isexpand) {
					ht
							.push("<ul  class='bbit-tree-node-ct'  style='z-index: 0; position: static; visibility: visible; top: auto; left: auto;'>");
					if (nd.ChildNodes) {
		...
		function check(item, state, type) {
			...
			if (item.render && pstate != item.checkstate) {
				...
				var et = $("#" + id + "_" + nid + "_cb");
				if (et.length == 1) {
					et.attr("src", dfop.cbiconpath
							+ dfop.icons[item.checkstate]);
				}
			}
		}
		//遍历子节点
			...
			if (et.tagName == "IMG") {
				...
				if ($(et).hasClass("bbit-tree-elbow-plus")
					...
					if (ul.hasClass("bbit-tree-node-ct")) {
						...
						if (item.complete) {
							item.ChildNodes != null
									&& asnybuild(item.ChildNodes, deep, path,
											ul, item);
						} else {
					...
					$(this).swapClass("bbit-tree-node-collapsed",
						...
						|| $(et).hasClass("bbit-tree-elbow-end-minus")) { //- 号需要收缩
					$(this).next().hide();
		...
		function reflash(itemId) {
			...
			if (node.length > 0) {
				...
				var isend = node.hasClass("bbit-tree-elbow-end")
						|| node.hasClass("bbit-tree-elbow-end-plus")
						|| node.hasClass("bbit-tree-elbow-end-minus");
		...
		function getck(items, c, fn) {
			for (var i = 0, l = items.length; i < l; i++) {
				(items[i].showcheck == true && items[i].checkstate == 1)
						&& c.push(fn(items[i]));
				if (items[i].ChildNodes != null
		...
		function getCkAndHalfCk(items, c, fn) {
			for (var i = 0, l = items.length; i < l; i++) {
				(items[i].showcheck == true && (items[i].checkstate == 1 || items[i].checkstate == 2))
						&& c.push(fn(items[i]));
				if (items[i].ChildNodes != null
						&& items[i].ChildNodes.length > 0) {
